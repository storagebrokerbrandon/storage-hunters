      ...
      <main className="flex-1">
        <section className="border-b border-navy-100 bg-gradient-to-b from-navy-50 to-white">
          <div className="mx-auto w-full max-w-6xl px-6 py-16 text-center sm:py-20">
            ...
            <h1 className="mx-auto mt-3 max-w-2xl text-3xl font-bold tracking-tight text-navy-900 sm:text-5xl">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {guides.map((guide) => (
                <Link
                  key={guide.href}
                  href={guide.href}
                  className="group rounded-lg border border-navy-100 bg-white p-8 transition-all hover:-translate-y-1 hover:border-navy-200 hover:shadow-lg hover:shadow-navy-900/10"
                >
                  <h2 className="text-xl font-semibold text-navy-900 group-hover:text-navy-600">
                    {guide.title}
                  </h2>
                  <p className="mt-3 leading-7 text-navy-800/70">
                    {guide.description}
                  </p>
                  <p className="mt-5 text-sm font-semibold text-navy-600">
                    Read the guide &rarr;
                  </p>
                </Link>
              ))}
            </div>
            <div className="mt-16 rounded-lg bg-navy-900 p-8 text-center sm:p-10">
              <h2 className="text-2xl font-bold tracking-tight text-white">
                Frameworks are the start. Deals are the goal.
              </h2>
              <p className="mx-auto mt-3 max-w-md text-base leading-7 text-navy-100/80">
                Book a free acquisition call and we&apos;ll apply all of this
                to your market and your buy box.
              </p>
              <Link
                href="/#book"
                className="mt-6 inline-block rounded-md bg-white px-6 py-3 text-sm font-semibold text-navy-900 transition-colors hover:bg-navy-50"
              >
